...
//NSLog(@"dx %f dy%f",dx,dy);
    ...
    if (self.isLeft)
    {
        [gameDelegate updateJoystikDegrees:degrees];
    }
    else
    {
        [gameDelegate updateRotationDegrees:degrees];
    }
}
...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if(location.x < -joystickRadius || location.x > joystickRadius || location.y < -joystickRadius || location.y > joystickRadius){
		...
		if(joystickRadiusSq > dSq){
            ...
            if (isLeft)
            {
                [gameDelegate startUpdate];
            }
            else
            {
                [gameDelegate startUpdateDegrees:degrees];
            }
...
- (void)ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    {
        [gameDelegate stopUpdateWithDegrees:degrees];
    }
}
